...
- UI draws edge to edge, but interactive or informational elements (buttons, map controls, attribution, text) must never sit under the status bar, navigation bar or a display cutout; offset them by `WindowInsets.safeDrawing`. System bar icons must stay readable: over the map, keep the status bar fully transparent with icons coloured to suit the map style (dark on a light map, like Google Maps); give the navigation bar a translucent scrim in the system theme's colour, with icons that follow that theme.
