...
- UI draws edge to edge, but interactive or informational elements (buttons, map controls, attribution, text) must never sit under the status bar, navigation bar or a display cutout; offset them by `WindowInsets.safeDrawing`. System bar icons must stay readable: give both bars the same translucent scrim in the system theme's colour (light or dark), with icons that follow that theme.
