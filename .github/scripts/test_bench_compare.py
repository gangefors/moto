...
BASE = {
    ...
    "curvy_route_ms_mean": 25.0,
    "curvy_route_ms_p95": 70.0,
    "loop_ms_mean": 120.0,
    "loop_ms_p95": 260.0,
    "match_ms_per_km": 0.3,
}
...
class CompareTest(unittest.TestCase):
    ...
    def test_a_metric_new_since_the_baseline_is_not_comparable(self):
        old = {k: v for k, v in BASE.items() if not k.startswith(("fav_", "curvy_", "loop_"))}
        lines, regressed = bc.compare(old, with_(fav_route_ms_mean=500.0, loop_ms_mean=900.0))
        self.assertEqual(regressed, [])
        self.assertEqual(sum("not comparable" in l for l in lines), 6)
