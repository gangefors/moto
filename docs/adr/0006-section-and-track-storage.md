...
- [ ] Section/tag/track schema with `user_version` migrations; tests for every migration and for crash-in-the-middle (transaction rollback). Sections done (schema 1), tracks done (schema 2); tags come with M1 step 6.
- [ ] Coarse FFI calls for sections, tags and tracks; Kotlin passes the database path in app-private storage. Sections done (`SectionStore`, `Engine.sectionBetween`); tracks done (track calls on `SectionStore`, `Engine.matchTrack`); the Kotlin side's crash-safe fix buffer is a text file per track in app storage, handed to the core every 30 fixes or 30 s.
