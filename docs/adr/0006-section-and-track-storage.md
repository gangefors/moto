...
- [x] GeoJSON export/import in the core with limits and corruption tests (R10): plain, .gz, .zip or .tar.gz, read in memory with size caps (32 MiB in, 64 MiB unpacked), strict types, overlap rules (the shorter of two fully overlapping sections is dropped), one transaction per import.
