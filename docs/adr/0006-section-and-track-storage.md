...
- [x] GeoJSON export/import in the core with limits and corruption tests (R10): plain, .gz or .zip (tar.gz dropped 2026-09-24: an export is one file), read in memory with size caps (32 MiB in, 64 MiB unpacked), strict types, overlap rules (the shorter of two fully overlapping sections is dropped), one transaction per import.
