...
- **Tables:** `sections` (id, rider_id, name, rating, direction, source, created/updated, status such as `ok` / `needs_rematch` / `unmatched`), `section_ways` (ordered OSM way id + node range per section), `section_geometry` (the section's own polyline, independent of the region file), `tags` (time, position, heading, speed, optional track), `tracks` and `track_points` (a recorded ride).
