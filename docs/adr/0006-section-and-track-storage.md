...
- [x] Saved routes (PRD P1): `routes` table (schema 5) with save, list, line, rename and delete on `SectionStore`; rows read back are validated. GPX files import as finished rides (`import_track_gpx`, one transaction).
