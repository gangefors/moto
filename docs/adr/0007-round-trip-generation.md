...
- **Other sets and a direction** (added 2026-09-25, Stefan): `LoopOptions { seed, bearing }`. Seed 0 is the standard set above, the same every time; any other seed turns the headings by up to one step and varies each candidate's spread (15–45°) and size (0.8–1.2), deterministically, so the app's Shuffle gives a new set that is the same for the same seed. A bearing fans the 12 candidates within 60° either side of it; when fewer than two loops fit that way, the best loops of any direction top the set up to two.
